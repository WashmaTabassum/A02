...
function addtoCart(bookID) {
  ...
  if (selectedBook) {
      ...
      if (existingBook) {
          existingBook.quantity++; // If book already exists, increment its quantity
      } else {
          selectedBook.quantity = 1; // If the book is not in the cart, add it with a quantity of 1
          cartItems.push(selectedBook);
      }

      localStorage.setItem('cartItems', JSON.stringify(cartItems)); // Update localStorage with the modified cart items
